...
public class PlantController {
    ...
    @GetMapping("/{plantId}")
    public ResponseEntity<Plant> getPlantById(@PathVariable("plantId") String plantId) {
 ...
 // This method maps to PUT requests to /api/plants/{id}
    @PutMapping("/{plantId}")
    public ResponseEntity<Plant> updatePlant(@PathVariable("plantId") String plantId, @RequestBody Plant plant) {

        // Find the plant with the given id from the plantRepository
        Optional<Plant> plantData = plantRepository.findById(plantId);
        ...
        if (plantData.isPresent()) {
            ...
            updatedPlant.setplantName(plant.getplantName()); // Update the plant name using setPlantName() method
    ...
    public ResponseEntity<HttpStatus> deletePlant(@PathVariable("plantId") String plantId) {
